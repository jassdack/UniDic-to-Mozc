...
**Mozc が変換できない約13.8万語を足す、UniDic 由来の語彙補強辞書**
...
>
> これは推測ではなく実測です。稼働中の Mozc に実際に変換させて測っています（[測定結果](#辞書品質の測定)）。

| 実測項目 | 値 |
| :--- | :--- |
| 本当に足している語 | **約13.8万語**（収録の51.5%） |
| Mozc が元から出せる語の重複 | 48.5% |
| 日常語124件の第1候補一致 | 辞書なし 87.1% → 辞書あり **85.5%** |

### 向いている場合

- 学術・専門用語、文語形、地名など、**一般的な辞書に載っていない語**を打つ
- `MVP` `CFA` `Gesellschaft` `ぐらむ→g` のような**略語・外来語・単位記号**をよく使う（この分類は Mozc の非収録率が93.8%で最も高い）
- 変換候補が多少増えても、語彙が足りないほうが困る

### 向いていない場合

- **日常語の変換精度を上げたい** — 実測では改善しませんでした。Mozc 単体で既に上位10件のカバー率が100%で、追加辞書が入り込む余地がありません
- **複合語を打ちたい** — UniDic は短単位辞書のため `構造体` `非同期` `廃藩置県` `万葉仮名` などは収録されていません
- **新語・ネットスラングが欲しい** — UniDic の基盤は2000〜2010年代の均衡コーパスです

人名（約8.7万語）は既定の配布物に含みません。同音異義語の候補を大きく圧迫する一方、主要な姓は地名・固有名詞としても収録されているため、外してもカバレッジは1件も落ちないことを実測で確認しています。人名が必要な方は、別配布の人名辞書を追加インポートしてください。

> [!NOTE]
> 📊 **[Mozc に大規模辞書を足すと変換は良くなるのか — 実測](docs/findings.md)**
> 本辞書に限らず、Mozc 拡張辞書一般に当てはまる測定結果をまとめています。
...
人名の除外によって**カバレッジは1件も落ちず**、競合だけが約25%減ります。

### 実変換精度（Windows のみ）

稼働中の `mozc_server` へ IPC で接続し、実際に変換させて辞書の有無を比較できます。
...
> [!IMPORTANT]
> **この辞書はトレードオフです。**
> 日常語では変換精度が**改善しませんでした**（辞書なしの時点で上位10件のカバー率が
> 100% であり、追加辞書が入り込む余地が無いため）。測定値は 2 件下がりましたが、
> この −2 は統計的にゼロと区別できません（結果が変わったのは4件のみ、McNemar 検定で p ≈ 0.63）。言えるのは「悪化した」ではなく**「改善は検出できなかった」**です。
> 一方で、辞書の約 45.7%（およそ12万語）は Mozc が元から出せない語であり、
> 稀な語彙を新たに打てるようになります。
>
> 逆に約 51% は Mozc が元から出せる語の重複登録で、競合を増やすだけです。
> これは `eval/scan_system_coverage.py` で洗い出し、`--exclude-list` で落とせます。

測定の前提と限界は [`eval/README.md`](eval/README.md) を参照してください。
