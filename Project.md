...
4. **ファイル分割**: 全データを一定語数ごとに分割し、連番付きのTSVファイル群として出力します。既定の分割幅は 100,000 語で、`--limit` で変更できます。
...
>
> したがって既定辞書 267,413 語は本来1ファイルに収まり、分割は必須ではありません。
> 既定の分割幅 100,000 は、配布実績のある保守的な値として当面維持しています。
