...
> [!WARNING]
> 下表は活用型マッピング修正**前**の生成物に基づく数値です。修正前は UniDic の活用型表記（`サ行変格`/`カ行変格`/`ラ行変格`）を
> IPAdic 表記（`サ変`/`カ変`/`ラ変`）で判定していたため、変格活用動詞がすべて「動詞一段」に落ちていました
> （動詞一段 13,868 + 動詞〇行五段 16,745 = 動詞全体 30,613 と一致し、サ変・カ変・ザ変・ラ変が0件であることが確認できます）。
> 修正版での再生成後に更新予定です。
