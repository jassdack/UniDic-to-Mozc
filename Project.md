...
当初は「入力体験の向上」を掲げていましたが、実測の結果この表現は正確ではないと分かりました。日常語の変換精度はむしろわずかに下がります（第1候補一致 87.1% → 85.5%）。Mozc 単体で既に上位10件のカバー率が100%であり、追加辞書が入り込む余地がそもそもありません。
...
> 日常語の第1候補一致率は 87.1% から 85.5% へ下がります。
...
- **本プロジェクトのアプローチ**: 入力ソースを学術コーパスのみに限定し、活用形を「基本形」に絞り込むことで、登録語数を抑えています。ただし**「ノイズを排除した」と言えるほどの効果は実測では確認できていません** — 日常語の変換精度はむしろわずかに下がります（「辞書品質の測定」参照）。完全ローカル（オフライン）で動作する静的辞書であるため、辞書自体が通信を行うことはありません。
...
> 本辞書を入れると実測でわずかに下がります（87.1% → 85.5%）。
