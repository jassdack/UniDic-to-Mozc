...
本プロジェクトにおける「目的（Why）」から「具体的な解決手段（How）」への階層構造を整理します。ネストが深くなる（インデントが下がる）ほど、抽象的な大目的から具体的な実装手段へとブレイクダウンされています。
...
以上の運用課題を全て解決するため、**「10万語ごとに分割されたユーザー辞書（TSV）」**そのものをOSSとして公開する。さらに、公開ユーザー辞書のソース明示（透明性）とハッカビリティ（改変の容易さ）を担保するため、**「変換プロセスを担うスクリプト（Python）」**も合わせてOSS公開する。
...
1. **ストリーム読み込み**: メモリ枯渇を防ぐため、CSVを一行ずつジェネレータ的に処理。
2. **ノイズ判定と足切り (Early Return)**:
   - 動詞や形容詞の活用形を検査し、「基本形（終止形）」以外を全てドロップ。実際の活用処理はIME側のネイティブエンジンに任せることで劇的な軽量化を実現。
   - 語種コードを参照し、「記号」「不可触（処理制約のある語）」などをドロップ。
3. **正規化 (Normalization)**: 表記（Surface）や語彙素（Lemma）に含まれる不要な全角英数字・記号を、事前コンパイルされた変換表を用いて高速に半角へ統一。
4. **読みのクレンジング (Phonetic Cleansing)**:
   - IME入力に最も適した「仮名形出現形（実際の発音に即した綴り）」を最優先で取得し、欠損がある場合は「語彙素読み」等へ段階的にフォールバック。
   - 抽出したカタカナを平仮名へ変換し、事前コンパイルされた正規表現でIME入力不可能な無効文字を削除。
5. **重複排除とコスト付与**: `(読み, 表記, 品詞)` のタプルをハッシュセットで管理して完全な重複を排除し、UniDic特有の「単語生起コスト（頻度スコア）」をコメント欄横に不可視メタデータとして付与。
...
- **課題**: 一般大衆向けであるため、専門分野の用語（IT、医学、地理など）や最新の口語表現のカバー率が低く、プロフェッショナルな執筆やコーディングの際には変換の「もたつき」や手動登録の手間が発生します。
...
- **課題**: 収集データが無差別であるため、極端な表記揺れや不適切な単語が上位に表示される「サジェスト汚染」が発生しやすくなります。また、入力内容に基づく学習・予測機能は、機密情報を取り扱う実務においてプライバシー上の懸念を伴います。
- **本プロジェクトのアプローチ**: 入力ソースを純粋な学術コーパスのみに限定し、さらに「基本形」に絞り込むことで、サジェストノイズを極限まで排除しました。また、完全ローカル（オフライン）で動作する静的辞書であるため、情報漏洩のリスクは皆無です。

### 6.3. プレミアムIME（ATOK 等）

- **特徴**: 有償（サブスクリプション等）で提供され、AIを活用した高度な文脈解析、同音異義語の解説、強力な校正支援機能を備えています。出版社編纂の国語辞典データセットを使った「正しい日本語（規範的）」を記述するための手厚いサポートが強みです。また、それらを組み合わせることができるカスタマイズ性も高いです。
- **課題**: 口語的な辞書や、若者言葉などの辞書が用意されている場合もありますが、規範的なアプローチを重視するため、ユーザーの生の思考に近い俗語や、生々しい記述的データへの対応は遅れる傾向にあります。また、システムがブラックボックス化されており、ユーザー自身による動作原理の検証やハック（改造）は不可能です。またクラウド変換などの機能についてはGoogle 日本語入力同様プライバシー上の懸念を伴います。
...
- **特徴**: WikipediaやWeb辞書など、多様なソースを無差別にマージして語彙数の最大化（数百万語規模）を志向した、過去のパワーユーザー向けプロジェクト群です。
- **課題**: 活用形を含めた膨大なデータをそのままシステム辞書に取り込むため、深刻なサジェスト汚染を引き起こしました。さらに、システム辞書を直接上書きするための「複雑なC++ビルドチェインへの依存」は参入利用の障壁が高く、そして多様なデータソースの混在による「ライセンスの非互換性（コンタミネーション）」が原因となり、主要プロジェクトは開発・公開の停止に追い込まれました。
...
## 補遺：日本語学および辞書学的視点に基づくアプローチの妥当性

本プロジェクトのデータ処理手法は、辞書学および日本語の形態論的アプローチとも一定の整合性を持っています。

### 1\. 辞書学における見出し語（Lemma）単位の整理

コーパス（CWJ/CSJ）は実際の言語使用の記録（記述的データ）ですが、辞書として機能させるためには情報の整理が必要です。本プロジェクトにおける「基本形への集約」処理は、様々な表層形から語彙素（Lemma）を抽出し、動詞や形容詞を終止形で立項するという辞書学の基本的なアプローチに則っています。

### 2\. 形態論的短単位から生成エンジンへの適合

UniDic は形態論に基づく「短単位」を基準に構築されており、文章の解析（分解）に適しています。一方、IMEの役割は入力を基にした文字列の生成（結合）です。本システムにおける品詞マッピング処理は、UniDicの短単位データを、Mozcの連接コスト計算に基づく生成モデルが適切に解釈できるよう変換する役割を果たしています。活用形を網羅せず基本形のみを提供し、実際の活用処理はIME側のエンジンに委ねることで、冗長性を排除した効率的な変換プロセスを実現しています。

### 3\. IMEというシステムの性質と「記述的データ」の親和性

IME（入力・変換エンジン）の最大の目的は、「ユーザーの脳内にある思考を、極力ストレスなくそのままテキスト化すること」にあります。「正しい日本語はこうあるべき」という規範的（Prescriptive）な辞典データのみに依存した場合、ユーザーが日常的に用いる口語表現や、変化の激しい技術用語などを入力した際に「意図した変換候補が出ない」という認知摩擦（Cognitive friction）が発生します。

本プロジェクトが採用したUniDic（CWJ/CSJ）は、「現代の日本人が実際にどう言葉を使っているか」を網羅した記述的（Descriptive）なデータセットです。これをIMEの変換辞書として用いることで、ユーザーの自然な言語感覚（共時的な言語実態）とシステムの出力が高度に一致し、利便性の高いタイピング体験を実現します。本プロジェクトのアプローチは、「入力レイヤーでは記述的データで柔軟に受容し、必要に応じた校正は別の規範的プロセスで行う」という、現代のテキスト処理における合理的な役割分担を体現しています。

---

## 使用データセット (Base Datasets)

本プロジェクトのシステム構築および実証においては、以下のデータセット（2025年12月31日リリース版）をベースとして使用しています。

- **現代書き言葉UniDic (UniDic-CWJ)**
  - ファイル名: `unidic-cwj-202512.zip`
  - リリース日: 2025-12-31
