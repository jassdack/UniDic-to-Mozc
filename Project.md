...
本システムでは、CWJ・CSJ 各 1,027,505 行（合計 2,055,010 行）の CSV に対して、以下のパイプライン処理を一括で適用しています（UniDic ver.2025.12 実測）。
...
1. **ストリーム読み込み**: `csv.reader` のジェネレータ処理により、入力 CSV（約 275 MB）を全読み込みせず一行ずつ処理します。ただし重複排除のためのハッシュセットは全件を保持するため、メモリ使用量は出力件数に比例します（実測 RSS: 変換 110 MB / 統合 284 MB、354,666 語時）。
2. **ノイズ判定と足切り (Early Return)**:
   ...
   - 語種コード（和/漢/外/混/固/記号/不明）を `pos_mapping.json` の `noise_goshu` で参照可能。既定は空（無効）。
     ...
     IMEに有用な語彙まで失われる。真の記号類は品詞（`補助記号`/`記号`）による判定で既に除外している。
...
- **コスト値の限界（削除判断には使えない）**:
  UniDic のコストは MeCab の連接コストを含む値であり、語ごとの純粋な出現頻度ではありません。実測すると、固有名詞にはクラス単位でほぼ同一の値が付与されています。

  | 品詞 | 語数 | 異なるコスト値 | 最頻値の占有率 |
  | :--- | ---: | ---: | ---: |
  | 固有名詞 | 12,974 | 2,051 | 47.4% |
  | 地名 | 25,679 | 3,534 | 36.0%（上位2値で66%） |
  | 人名 | 87,253 | 6,397 | 26.2%（上位3値で56%） |

  つまり**著名な固有名詞と無名な固有名詞をコストで区別することはできません**。加えて、品詞別の中央値は動詞が最も高く（動詞一段 13,795 / 動詞ラ行五段 13,946）、固有名詞系 9,431・一般語彙 8,887 を上回ります。語彙が多いクラスほどコストが高くなるためです。

  したがって「一定コスト以上の語を足切りする」という設計は成立しません（動詞から先に消えます）。**コストは同一読み内の並べ替えには有効ですが、語の取捨選択の基準としては使えない**という前提で扱ってください。
...
4. **組込ディープマッピング (Built-in Logic)**:
   ...
   - 例えば `p1=動詞` の場合、さらに `cType`（活用型）を文字列解析し、「動詞〇行五段」「動詞サ変」「動詞一段」など、Mozc が持つ15種類の動詞タグすべてへ分配します（`config/mozc_pos_list.md` 参照）。`動詞カ行五段` `名詞サ変` `名詞形動` `人名` といった接続情報を保持したまま変換することで、解析用データを IME 用の入力補助データとして機能させています。この分配が意図どおり働いていることは、活用型ごとの回帰テストで担保しています（11章「検証と回帰テスト」を参照）。
...
## 5\. CWJおよびCSJの統合によるコスト推定の改善

本システムは「現代書き言葉均衡コーパス（CWJ）」と「日本語話し言葉コーパス（CSJ）」の両方を入力とし、重複を排除して統合しています。

> [!IMPORTANT]
> **統合によって語彙は増えません。** UniDic の CWJ と CSJ は同一の語彙集合を共有しており、異なるのは各コーパスで学習された単語生起コストのみです。
>
> 実測（ver.2025.12）: CWJ 単独 354,666 語 / CSJ 単独 354,666 語 / 統合後 354,666 語。統合後の件数が各単独の件数と一致することから、両者の語彙集合が完全に同一であることが確認できます。

統合の意義はコスト推定の改善にあります。本システムは同一の見出しに対して `min(コスト_CWJ, コスト_CSJ)` を採用するため、**書き言葉・話し言葉のいずれかで頻出する語は、その頻出性が反映された低いコストを得ます**。これにより、書き言葉寄りの統計だけでは埋もれる口語的な語（およびその逆）が、同音の候補群の中で上位に配置されます。

語彙そのものの網羅性は UniDic 単体の語彙設計に由来するものであり、本プロジェクトの統合処理による寄与ではありません。

## 6\. 既存環境および競合システムとの比較

本アプローチが既存の日本語入力環境において独自の価値を持つ背景を明確にするため、代表的なIME環境および先行プロジェクトとの設計方針を比較整理します。

> [!NOTE]
> 本節は設計思想の対比であり、変換精度の実測に基づく比較ではありません。本プロジェクトは変換精度を測る指標をまだ定義できていません（12章「既知の限界」を参照）。
...
- **本プロジェクトのアプローチ**: UniDic に裏打ちされた 354,666 語の見出しにより、学術用語から口語表現までを収録。OS標準辞書の「痒い所に手が届かない」語彙不足を、ローカル環境のままに解消します。
...
- **本プロジェクトのアプローチ**: 入力ソースを純粋な学術コーパスのみに限定し、さらに「基本形」に絞り込むことで、サジェストノイズを極限まで排除しました。また、本辞書は完全ローカルで動作する静的な TSV であり、辞書データ自体は外部送信を一切伴いません。ただしこれは辞書が持ち込むリスクについての記述であり、IME 本体やOSが備える他の機能まで保証するものではありません。
...
- **「データはテキストとして扱え (Store data in flat text files)」**: 最終出力をバイナリ（.dic等）にコンパイルするのではなく、普遍的なインターフェースであるプレーンなタブ区切りテキスト（TSV）として出力します。これにより、ユーザーは grep や awk といった標準的なUNIXコマンドを用いて辞書データを自由に検索・加工・再利用できます。
  > [!NOTE]
  > ただし現時点では、両スクリプトとも入出力にファイルパスを要求し、標準入出力には対応していません（`convert_unidic.py < lex.csv > out.tsv` は動作しません）。「テキスト形式で保存する」原則は満たしていますが、「フィルタとしてパイプに繋げる」水準には達していません。
- **「シンプルさは正しさに優先する (Worse is Better)」**: システム辞書を直接ハックしてビルドする従来のアプローチは、パフォーマンス面では理論上「完璧」かもしれませんが、複雑なビルドツールチェーンへの依存という脆さを抱えていました。本プロジェクトは「ユーザー辞書の10万語制限を、ファイルを単純分割することで回避する」という泥臭くも極めてシンプルな解決策を採用しました。このシンプリシティこそが、OSのアップデート等に左右されず永続的に動き続けるための条件です。
  > [!NOTE]
  > 「Worse is Better」は UNIX 哲学の項目ではなく、Richard Gabriel が1989年に MIT/Stanford 流の「正しいこと (The Right Thing)」と対比して提示した別系統の概念です（実装の単純さがインターフェースの完全性に優先する、という主張）。本プロジェクトの選択を説明する概念として適切なため、出典を明示したうえで併記しています。
...
**【品詞マッピング統計（抽出元データとの比較）】** 抽出元である UniDic CWJ ver.2025.12 の初期語彙（1,027,505 語）と、各種フィルタを実行した Mozc 出力語彙（354,666 語）の比較です。

| Mozc 指定品詞       | 出力数（本辞書） |   構成比 | \[参考\] UniDic 元データ要素数 | 削減・最適化の要因                                   |
| :------------------ | ---------------: | -------: | -----------------------------: | :--------------------------------------------------- |
| **名詞**            |          158,907 |   44.80% |                        327,910 | サ変・人名・地名等への細分化分離による減少           |
| **人名**            |           87,253 |   24.60% |                             \- | 一般名詞・固有名詞からの精密抽出                     |
| **地名**            |           25,679 |    7.24% |                             \- | 同上                                                 |
| **動詞全体**        |           30,614 |    8.63% |                        617,288 | **「基本形（終止形）」への強制集約による劇的な圧縮** |
| 　動詞（〇行五段）  |           18,433 |    5.20% |                             \- | 活用型から9行別への完全分配                          |
| 　動詞一段          |           11,641 |    3.28% |                             \- | 上一段・下一段および文語二段からの集約               |
| 　動詞サ変          |              358 |    0.10% |                             \- | 活用型「サ行変格」からの分配                         |
| 　動詞ザ変          |              106 |    0.03% |                             \- | 「〜ずる」型の語尾判定による分離                     |
| 　動詞カ変          |               47 |    0.01% |                             \- | 活用型「カ行変格」からの分配                         |
| 　動詞ハ行四段      |               21 |    0.01% |                             \- | 文語四段ハ行の専用品詞へ割当                         |
| 　動詞ラ変          |                8 |    0.00% |                             \- | 活用型「文語ラ行変格」からの分配                     |
| **名詞サ変**        |           15,428 |    4.35% |                             \- | 名詞からのサ変可能語の分離                           |
| **固有名詞**        |           12,974 |    3.66% |                             \- | 人名・地名以外の固有名詞                             |
| **アルファベット**  |            6,019 |    1.70% |                             \- | 全角英字を半角化した上での判定                       |
| **形容詞**          |            4,983 |    1.40% |                         53,561 | 基本形への集約                                       |
| **副詞**            |            4,817 |    1.36% |                         11,510 | 記号混じり等のノイズ排除                             |
| **名詞形動**        |            3,559 |    1.00% |                          4,570 | Mozc仕様の「名詞形動」へマッピング。語の活用を可能に |
| **助数詞**          |            1,235 |    0.35% |                             \- | 名詞・接尾辞からの再配置                             |
| **接尾一般**        |            1,075 |    0.30% |                          2,333 | 人名・地名・助数詞への再配置                         |
| **感動詞**          |              873 |    0.25% |                          3,129 | 日常入力に不要なノイズ排除                           |
| **その他特殊/付属** |            1,250 |    0.35% |                             \- | 数・接頭語・独立語・連体詞・接続詞                   |
| **合計**            |      **354,666** | **100%** |            **1,027,505** | （話し言葉 CSJ を含むマージ後の最終件数）            |

---

## 11\. 検証と回帰テスト

本プロジェクトはノイズの少なさと品詞マッピングの正確さを設計上の主張の中心に置いています。しかし主張は、それが崩れたときに気づく仕組みとセットでなければ維持できません。この節はその仕組みについて記述します。

### 11.1. なぜ必要か（実際に起きたこと）

ver.1.0.0 では、すべての変格活用動詞が「動詞一段」として出力されていました。UniDic の活用型は `サ行変格` `カ行変格` `ラ行変格` ですが、変換スクリプトが IPAdic 表記の `サ変` `カ変` `ラ変` で判定していたためです。Mozc は品詞に従って活用させるため、「勉強する」が一段動詞として登録されると *勉強すない* のような候補が生成されます。

注目すべきは、**その証拠が本ドキュメントの統計表の中にあった**ことです。

```
動詞一段 13,868 + 動詞〇行五段 16,745 = 動詞全体 30,613
```

合計が完全に閉じており、サ変・カ変・ザ変・ラ変の出力が0件であることを示していました。列を縦に足せば気づけたはずですが、テストが無かったため誰も足しませんでした。**検証の仕組みを持たない品質主張は、破綻しても破綻したことが分からない**というのが、このプロジェクトが実際に踏んだ落とし穴です。

### 11.2. 現在の検証内容

`tests/test_converter.py` に標準ライブラリのみの回帰テストを置き、GitHub Actions で Python 3.9 / 3.13 に対して実行しています。外部依存はありません。
...
| 対象 | 内容 |
| :--- | :--- |
| 活用型マッピング | 18種の `cType` について、JSON 経路・組込経路の双方が期待どおりの Mozc 品詞を返すこと |
| 経路の一致 | 設定ファイルの有無で結論が変わらないこと |
| カラム位置 | 語種が `row[16]` であること（`row[17]` は語頭変化型） |
| 品詞の優先順位 | UniDicの具体的な品詞 > アルファベット > 一般フォールバック の順が保たれること |
| TSV 整合性 | CSV のクォート内のタブ・改行が列を壊さないこと、改行が実在しない語を生まないこと |
| ノイズ判定 | 終止形以外の活用形が除外されること、英字略語が既定で残ること |
| 分割・統合 | 10万語での分割、低コスト側の優先、オプション位置の非依存 |

### 11.3. 統計表の自己検証

品詞統計表（「品詞体系のマッピングロジックと統計結果」）は、単なる紹介ではなく**検算可能な資料**として扱ってください。動詞の内訳の合計が動詞全体と一致するか、構成比の合計が100%になるかは、マッピングの破綻を検出する最も安価な手段です。

---

## 12\. 既知の限界

本ドキュメントは解決した課題を中心に記述してきましたが、未解決の課題も同じだけ重要です。

### 12.1. 「入力体験の向上」を測る指標が無い

冒頭で本プロジェクトの目的を「語彙数の増加ではなく入力体験の向上」と定義しました。しかし本ドキュメントの統計はすべて**辞書の組成**（語彙数・品詞分布・削減率）であり、入力体験そのものを測った数値は一つもありません。6章の比較も設計方針の対比であって、変換精度の実測ではありません。

結果として、宣言した目的ではなく測定可能な手段のほうを最適化する構造になっています。「サジェスト汚染」を最大の敵と位置づけながら、汚染の度合いを定義していない点は、本プロジェクトの設計思想における最大の空白です。

### 12.2. 固有名詞が辞書の3分の1以上を占める
...
想定ユーザーを「一般的な事務作業やビジネスコミュニケーション」と定義している一方で、87,253 件の人名が候補空間に入ることの影響は検討していません。「引き算」の思想は活用形と記号には適用されていますが、低頻度の固有名詞には適用されていません。

なお、5.1 で述べたとおりコストによる足切りはこの用途に使えないため、対処するなら別の基準が必要です。

### 12.3. 候補の集中（実測）

読みごとの候補数を実測すると、汚染は存在するものの**極端に集中**しています。
...
読みの3分の2は候補1件で、競合が発生しません。問題は 2,752 の読みに凝縮されており、その中身は漢字表記の人名です（`こうじ` 133候補中105が人名、`ゆうき` 102候補中85が人名、`ひろし` 87候補すべてが人名）。

一方で**固有名詞の 74.9% は一般語彙と読みが衝突していません**。害があるのは残り 25.1%（31,602語）に限られます。

対策として固有名詞を別ファイルに分離した場合の試算は以下のとおりです。データを1語も失わずに混雑を半減できますが、**候補数の削減が実際に入力体験を改善するかは未検証です**（12.1 の指標が無いため）。

| 方式 | 語数 | 10候補以上の読み | 該当語数 | 最大候補数 |
| :--- | ---: | ---: | ---: | ---: |
| 現状（一体配布） | 354,666 | 2,752 | 46,690 | 133 |
| 固有名詞を分離（一般側） | 228,760 | 1,203 | 17,852 | 82 |
| 　同上（固有名詞側） | 125,906 | 911 | 16,318 | 109 |

### 12.4. 文語・方言の扱いは近似である

Mozc には文語二段活用や文語四段活用（ハ行を除く）に対応する品詞がありません。本システムはこれらを現代語の品詞へ近似的にマッピングしています（文語二段 → 動詞一段、文語四段 → 対応する行の五段）。9章で述べた「便宜的マッピング」の方針に沿った選択ですが、国語学的な正確さは犠牲になっています。

### 12.5. 企業配備の想定と配布形態の距離

想定ユースケースに「企業が従業員に支給貸与するPCにユーザー辞書導入済みのものを使用する」を挙げていますが、実際の導入は GUI での手動インポート（複数ファイル）であり、差分更新やバージョン確認の手段がありません。辞書を更新する際は既存の辞書を削除して再インポートする必要があります。個人利用には十分ですが、規模のある配備を想定用途として維持するなら、この距離は埋める必要があります。

---

## 13\. 補遺：日本語学および辞書学的視点に基づくアプローチの妥当性検証
...
- **解決**: 本プロジェクトでは、UniDic の活用形カラム（`cForm`）を用いて「終止形-一般」以外の行をすべて除外し、動詞や形容詞を「基本形（終止形）」へ強制集約しています。出力される見出しはその行の書字形出現形であり、語彙素（Lemma）そのものではありませんが、終止形に限定した結果として辞書フォームと一致します。これは、「見出し語は活用を除いた抽象的な一単位であるべき」という辞書学の基礎原則に則っており、IMEにおいてユーザーが最も自然に想起する変換候補（辞書フォーム）での立項を実現しています。
